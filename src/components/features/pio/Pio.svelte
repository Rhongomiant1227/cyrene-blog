<script lang="ts">
	...
	const pioOptions = {
		...
		model: (config?.models ?? pioConfig.models ?? ["/pio/models/pio/model.json"]).map(
			(modelPath) => (modelPath.startsWith("/") ? url(modelPath) : modelPath),
		),
	};
	...
	function loadPioAssets() {
		...
		const loadWithIdle = () => {
			loadScript(url("/pio/static/l2d.js"), "pio-l2d-script")
				.then(() => loadScript(url("/pio/static/pio.js"), "pio-main-script"))
				.then(() => {
					setTimeout(initPio, 100);
				})
				.catch((error) => {
